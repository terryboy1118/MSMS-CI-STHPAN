    ...
    def get_relation(self, flag):
        ...
        if self.graph:
            ...
            # dtw relation
                ...
                if self.ci == 1:
                    if flag == 'train':
                        hyperedge_index = np.load(os.path.join('/home/adam/CI-STHPAN-main/CI-STHPAN_self_supervised/nyse_all/nyse_relation/dtw_train_matrix_binary.npy'))
                        
                    else:
                        hyperedge_index = np.load(os.path.join('/home/adam/CI-STHPAN-main/CI-STHPAN_self_supervised/nyse_all/nyse_relation/dtw_valid_matrix_binary.npy'))
                else:
                    if flag == 'train':
            ...
            #MSMSDTW-min
                ...
                if self.ci == 1:
                    if flag == 'train':
                        file_path = f"/home/adam/MSMS-CI-STHPAN/CI-STHPAN_self_supervised/src/data/datasets/stock/relation/MSMSDTW-min/dtw_train_matrix_min_{self.market.lower()}_binary.npy"
                        hyperedge_index = np.load(file_path)
                    else:
                        file_path = f"/home/adam/MSMS-CI-STHPAN/CI-STHPAN_self_supervised/src/data/datasets/stock/relation/MSMSDTW-min/dtw_valid_matrix_min_{self.market.lower()}_binary.npy"
                        hyperedge_index = np.load(file_path)
                # channel mixing
                else:
            ...
            if self.ci == 1:
                ...
                #print("舊 relation 非零數量:", np.count_nonzero(hyperedge_index))
                hyperedge_index_ci = []
                for m in range(self.dls.vars):
